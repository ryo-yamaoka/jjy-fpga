...
| 4 | PC 定期同期（UART 受信回路 + PC 側常駐スクリプト） | 動作確認済（電波時計が PC 時刻に同期、RTC への書き戻しも確認）。常駐スクリプトの長時間運用テストは未実施 |
| 5 | RX8900 RTC モジュール接続による時刻精度向上（I2C マスタ + 時刻ソース調停 + FOUT 1Hz 秒境界規律） | **完了**。完了条件 3 件すべて達成（PC 無しで一昼夜同期継続・電源断復帰・PC からの RTC 書き込み） |
| 6 | LC 共振回路・トランジスタバッファによるアンテナ改良（1〜2m 到達） | **完了**。2.0m での同期成立を実測で確認 |
| 7 | GPS 時刻源の組み込み（時刻源の完全自立） | 未着手 |

> 旧 Step 7（筐体化・常時運用化）は 2026-07 の設計改訂で廃止した。乾電池による独立電源化も検討したが持続時間が非現実的なため見送り、電源は USB 常時給電を継続する。経緯は設計ドキュメント §9.8 冒頭・§9.9 を参照。
...
Step 5 では AE-RX8900 RTC モジュール・CR2032（+ 電池ホルダ）・1N5817 ショットキーダイオード・100Ω 抵抗・10kΩ 抵抗 ×2（AE-RX8900 の J1 ショートで代替可）、Step 6 では共振用コンデンサ・トリマコンデンサ・トランジスタ（2SC1815 等）・抵抗等が追加で必要になる。詳細は設計ドキュメント §9.6 および §9.7 を参照。
...
| `led_rtc` | 16 | 3（1.8V） | LVCMOS18 | LED6: RX8900 の時刻未確立（VLF・未接続・初期化失敗）で点灯 |
| `sda` | **26** | 2（3.3V） | LVCMOS33 | **RX8900 SDA（I2C、オープンドレイン + 外付け 10kΩ プルアップ）** |
| `scl` | **27** | 2（3.3V） | LVCMOS33 | **RX8900 SCL（同上）** |
| `fout_1hz` | **28** | 2（3.3V） | LVCMOS33 | **RX8900 FOUT（1Hz 秒境界規律入力）** |

ピン **29**（GPS UART 受信）と **30**（GPS 1PPS）は Step 7（GPS 時刻源の組み込み）用に予約しており、現時点では未配線・未使用である。
...
5. 起動時の初期時刻は `hdl/src/jjy_top.sv` の `hour_tens` / `hour_ones` 等のリセット値で決まる。現状は **2000-01-01（土）00:00:00** であり、これは「時刻がまだ確立していない」ことを一目で示すための**意図的にあり得ない値**である。電波時計がこの時刻に同期した場合、RTC からのロードにも PC 同期にも失敗していることを意味する（LED6 も点灯しているはず）。RX8900 を接続している場合（§3.11）は起動直後に RTC の時刻で上書きされる。
...
### 3.11 RX8900 RTC の接続（Step 5 相当）

Step 5 では、高精度 RTC（秋月電子 AE-RX8900、RX8900SA 搭載）を I2C 接続し、PC 未接続でも FPGA 単独で正確な時刻を維持できるようにする。CR2032 によるバックアップで電源断後も計時が継続する。
...
| 3.3V | → | VDD (8) | 電源 |
| GND | → | GND (4) | グランド |
| ピン 26（`sda`） | ↔ | SDA (5) | 10kΩ プルアップを 3.3V へ |
| ピン 27（`scl`） | → | SCL (6) | 10kΩ プルアップを 3.3V へ |
| ピン 28（`fout_1hz`） | ← | FOUT (2) | 1Hz 秒境界規律信号 |
| 3.3V | → | FOE (1) | H 固定で FOUT 出力を有効化 |
| （未接続） | — | /INT (3) | 将来のアラーム用に開放 |
| — | — | VBAT (7) | CR2032 バックアップ回路へ（下記） |
...
- I2C プルアップは外付け 10kΩ ×2 **または** AE-RX8900 の J1 ショートの**どちらか一方のみ**。
- CR2032 は一次電池であり充電厳禁。ダイオードの向き（アノード=電池側）を必ず確認する。FPGA 側でも Backup Function Register への設定で充電を無効化している（二重の保険）。
- すべて 3.3V で統一する。5V を掛けないこと。

#### 3.11.2 動作

- **起動時**：電源投入から約 0.5 秒後に FPGA が RX8900 を初期化（FOUT を 1Hz に設定、充電無効化）し、時刻が有効（VLF=0）なら FOUT エッジに整列して時刻を読み出し、その時刻で JJY 送出を開始する。PC からの時刻設定は不要。
- **初回起動・電池切れ（VLF=1）**：RTC の時刻を信用せず、**LED6 が点灯**して PC からの時刻設定（§3.10）を待つ。時刻設定を受信すると内部時刻と RX8900 の両方が更新され、LED6 が消灯する。
- **PC 同期との併用**：Step 4 のプロトコルで時刻を受信するたびに RX8900 へも書き戻される。PC を繋いでいる間は NTP 精度、外せば RX8900 精度（月差 ±13 秒相当）へシームレスに縮退する。
- **秒境界規律**：RX8900 の FOUT（1Hz）エッジで内部の秒位相を常時再同期するため、オンボード水晶（±30ppm 程度）のドリフトは蓄積しない。1 時間ごとに時刻レジスタの再読出も行う。FOUT が途絶した場合は自動でフリーランに縮退し送出は継続する。

詳細な設計は設計ドキュメント §9.6、実装知見（レジスタの罠・規律ロジックの設計理由）は [docs/knowledge.md §11](docs/knowledge.md) を参照。

#### 3.11.3 シミュレーションによるロジック検証（任意）

RX8900 を模した I2C スレーブモデル入りテストベンチで、初期化・時刻読出・書き戻しのシーケンスを検証できる。
...
1. **増幅回路を追加してはならない。**
   トランジスタバッファ、オペアンプ、RF パワーアンプ等を用いて出力を増強した場合、容易に微弱無線局の上限を超える可能性がある。設計ドキュメント §9.7 で言及するトランジスタバッファについても、追加する場合は到達距離の最低限の確保を目的とし、過剰な駆動を避けること。
