...
5. 起動時の初期時刻は `hdl/src/jjy_top.sv` の `hour_tens` / `hour_ones` 等のリセット値で決まる（現状は **12:00** 固定、日付は 2026-04-13 月曜日固定）。同期した電波時計の表示が起動からの経過分を含めてこの値の付近になっていれば正常である。なお RX8900 を接続している場合（§3.11）は起動直後に RTC の時刻で上書きされる。
