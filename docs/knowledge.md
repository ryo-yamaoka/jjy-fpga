...
- `.cst` は `PULL_MODE=NONE` を指定する。プルアップは外付け 10kΩ **または** AE-RX8900 の J1 ショートの一方のみ（両方併用すると合成抵抗が下がりすぎる）。GW1N の内部プルアップ（約 50kΩ 相当）を第三のプルとして重ねない
...
警告。RX8900（エプソン ETM45J アプリケーションマニュアル）で実装時に注意が必要だった点：
...
4. **充電無効化は Backup Function Register(0x18) ← 0x0C**：VDETOFF(bit3)=1 + SWOFF(bit2)=1。一次電池（CR2032）+ 外付け保護ダイオード構成での推奨設定。ハードウェアのダイオードが主保護で、この設定は補助（初回実機投入時にデータシートで要再確認）
